...
public:
  static void AddSugggestedProcedures(
      ParsedRoute &parsedRoute, const std::string &origin,
    ...
    if (parsedRoute.departureRunway.has_value() && !waypointIds.empty()) {
      // We have a departure runway but no SID - find a SID that matches both
      // the runway and first waypoint
      ...
      // Find SIDs for this airport that match both the runway and first
      // waypoint
        ...
        if (it->second.icao == origin && it->second.type == PROCEDURE_SID &&
          ...
          for (const auto &procWpt : it->second.waypoints) {
            if (procWpt.getIdentifier() == firstWaypoint) {
              // Found a procedure that matches both the runway and the first
              // waypoint
      ...
      auto sidSuggestion =
          airportConfigurator->FindBestSID(origin, waypointIds);
      if (sidSuggestion) {
        ...
        parsedRoute.errors.push_back({ParsingErrorType::NO_PROCEDURE_FOUND,
                                      "Suggesting SID " + procedureName +
                                          " for runway " + sidSuggestion->first,
                                      0, // Index 0 (start of route)
                                      origin, ParsingErrorLevel::INFO});
      }
    }
    ...
    if (parsedRoute.arrivalRunway.has_value() && !waypointIds.empty()) {
      ...
      for (auto it = procedures.begin(); it != procedures.end(); ++it) {
        if (it->second.icao == destination &&
            it->second.type == PROCEDURE_STAR && it->second.runway == runway) {

          // Check if any of this procedure's waypoints match the last route
          // waypoint
          for (const auto &procWpt : it->second.waypoints) {
            if (procWpt.getIdentifier() == lastWaypoint) {
              // Found a procedure that matches both the runway and the last
              // waypoint
      ...
      if (starSuggestion) {
        ...
        parsedRoute.errors.push_back(
            {ParsingErrorType::NO_PROCEDURE_FOUND,
             "Suggesting STAR " + procedureName + " for runway " +
  ...
  static FoundProcedure FindProcedure(const std::string &token,
    ...
    bool isProcedurePattern = std::regex_match(
        procedureToken, std::regex("[A-Z]{2,5}\\d{1,2}[A-Z]?"));

    // Check if this is an airport code pattern (exactly 4 letters)
    bool isAirportPattern =
        (procedureToken.length() == 4 &&
      ...
      if (!matchingProcedures.empty() && runway) {
        ...
        return FoundProcedure{
            ...
            {ParsingError{
                ...
                fmt::format(
                    "No matching runway {} found for procedure {} at {}, "
                    "ignoring confirmed {}",
                    runway.value_or("N/A"), procedureToken, anchorIcao,
                    typeStr),
